...
public final class CodingMemoryPromptSections {
    ...
    private static final String CODING_MEMORY_READ_ONLY_CN = """
        # coding memory（只读）
        位于 `%s`。用 coding_memory_read 读取。不允许写入。
        """;
    ...
    private static final String CODING_MEMORY_READ_ONLY_EN = """
        ...
        At `%s`. Use coding_memory_read to read. No writing allowed.
        """;

    private static final String CODING_MEMORY_PROMPT_CN = """
        # coding memory

        你有一个基于文件的持久化记忆系统，位于 `%s`。该目录已存在，直接用 coding_memory_write 写入。
    ...
    private static final String CODING_MEMORY_PROMPT_EN = """
        ...
        You have a persistent, file-based memory system at `%s`. Write directly with coding_memory_write.
        ...
        |------|--------------|--------------|
        | user | Role,\
        goals, technical background, preferences | When you learn about user identity or preferences |



        | feedback | Corrections or confirmations of your approach | "don't do X" or confirms approach worked. \
        Include why |
        | project | Deadlines,\
        decisions not derivable from code | Who does what, why. Relative dates → absolute |


        | reference | Pointers to external systems (Jira,\
        Grafana, Slack) | When you learn about external resource locations |
        ...
        **Examples:**
        ...
        - **Edit existing**: Use `coding_memory_edit` to isReplace specific text (old_text → new_text)
        ...
        - `conflict_detected: true` + `conflicting_files: ["file.md"]` means semantic conflict with existing \
        memories
        - When conflict is detected,\
        the file will still be written (append mode) or created (create mode), but conflict info is returned
    ...
    public static PromptSection buildCodingMemorySection(String language, boolean isReadOnly, String memoryDir) {
        ...
        String template = isReadOnly
                ...
                : ("cn".equals(lang) ? CODING_MEMORY_PROMPT_CN : CODING_MEMORY_PROMPT_EN);
        return new PromptSection("memory", Map.of(lang, template.formatted(dir)), PRIORITY);
    }
}
