 ...
 * 中文提示词模板常量
 * <p>
 * Mirrors Python's {@code openjiuwen.dev_tools.prompt_builder.builder.prompt_zh}
 * 
 * @since 0.1.7
 */
public final class PromptTemplatesZh {
    ...
    public static final PromptTemplate PROMPT_FEEDBACK_INTENT_TEMPLATE =
        PromptTemplate.builder().content(List.of(new UserMessage("""
                ## 人设
                你是一个高效且精准的Prompt优化助手。用户将提供一个原始Prompt和用户的反馈信息。根据用户的反馈，判断该反馈是否有价值，能否进入后续的Prompt修改流程，并优化反馈信息的 \
                表达，使其更加清晰。同时，根据用户反馈，联想可能的其他优化方向，并提供相关的建议。
                ...
                - "optimization_directions"：根据用户反馈信息联想其他可能的优化方向，并为用户提供有价值的优化反馈信息。例如，提示用户可以思考其他细节或目标，进一步提升P \
                rompt质量。
                """))).build();
    ...
    public static final PromptTemplate PROMPT_FEEDBACK_GENERAL_TEMPLATE =
        PromptTemplate.builder().content(List.of(new UserMessage("""
                ...
                ## 任务描述
                现在你需要根据用户提出的反馈意见，对Prompt进行修改，请注意，你只会对Prompt做小的修改，而不是重写整个Prompt。因此，你需要在尽可能保持Prompt的原始语义的基础上 \
                ，将用户的反馈意见纳入其修改中。需要注意的是，Prompt中可能会包含占位符，它们以双花括弧的形式展现，你不可以对占位符本身进行增加、删除或修改。
    ...
    public static final PromptTemplate PROMPT_BAD_CASE_ANALYZE_TEMPLATE =
        PromptTemplate.builder().content(List.of(new UserMessage("""
                ...
                1. **分析整体反例输出intent内容**：如果反例的内容没有实际意义或者对改进原提示词完全没有帮助，那么输出`false`；如果反馈有价值，返回`true`，并将值包含在`< \
                intent>`和`</intent>`标签内
                ...
                3. **为每个反例生成具体的反馈**：将每个反馈包含在 `<feedback>` 和 `</feedback>` 标签内。每个反馈应包含：
                    ...
                    * 解释与提示词措辞或指令相关的可能原因。
                    * 提出针对该问题改进提示词的具体建议。
                4. **创建反馈的简洁总结**：在分析所有反例后，提供关键问题和建议改进的总结。将总结包含在 `<summary>` 和 `</summary>` \
                标签内。总结应将各个反馈综合成改进提示词的总体建议。重点关注方法论，而不是具体细节，并尽量保持简洁。
