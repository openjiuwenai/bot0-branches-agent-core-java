...
/**
 * ReMe Prompt templates.
 * <p>
 * Mirrors Python's {@code openjiuwen.extensions.context_evolver.summary.task.reme.prompt.ReMePrompts}.
 * 
 * @since 0.1.7
 */
public class ReMePrompts {
    /**
     * COMPARATIVE_MEMORY_PROMPT.
     * 
     * @since 0.1.7
     */
    public static final String COMPARATIVE_MEMORY_PROMPT = """
        ...
        step level.
        Focus on what made the higher-scoring approach more effective, even when both approaches may have had \
        partial success.
    ...
    /**
     * SUCCESS_MEMORY_PROMPT.
     * 
     * @since 0.1.7
     */
    public static final String SUCCESS_MEMORY_PROMPT = """
        You are an expert AI analyst reviewing successful step sequences from an AI agent execution.

        Your task is to extract reusable, actionable step-level task memories that can guide future agent \
        executions.
     ...
     * @since 0.1.7
     */
    public static final String FAILURE_MEMORY_PROMPT = """
        You are an expert AI analyst reviewing failed step sequences from an AI agent execution.

        Your task is to extract learning task memories from failures to prevent similar mistakes in future \
        executions.
    ...
    /**
     * COMPARATIVE_ALL_MEMORY_PROMPT.
     * 
     * @since 0.1.7
     */
    public static final String COMPARATIVE_ALL_MEMORY_PROMPT = """
        You are an expert AI analyst comparing multiple step sequences which might be successful or failed to \
        extract differential insights.

        Your task is to compare and contrast these trajectories to identify the most useful and generalizable \
        strategies as
        memory items using self-contrast reasoning. Focus on critical decision points, technique variations, \
        and approach differences.
        ...
        "experience": "Detailed comparison highlighting why the successful approach works better",
     ...
     * @since 0.1.7
     */
    public static final String MEMORY_VALIDATION_PROMPT = """
        You are an expert AI analyst tasked with validating the quality and usefulness of extracted step-level \
        task memories.

        Your task is to assess whether the extracted task memory is actionable, accurate, and valuable for \
